...
def procesar_patagonia_formato_3(archivo_pdf):
    """Procesa extractos de Banco Patagonia - Formato 3 (Estado de Cuentas Unificado)"""
    st.info("Procesando archivo del Banco Patagonia (Formato 3)...")
    try:
        archivo_pdf.seek(0)
        ...
        for line in lineas:
            m_cuit = re.match(r'C\.U\.I\.[TL]\.\s+\d+\s+(.+)', line.strip())
            if m_cuit:
                titular = m_cuit.group(1).strip()
                break

        # 2. DETECTAR SECCIONES DE CUENTAS
        # Formato libre: "<TIPO CUENTA> <NUMERO> SUBCTA <n> SUC <n> CBU: <cbu>"
        # El estado de cuentas unificado puede listar varios productos
        # (Cuenta Corriente, Caja de Ahorros, etc.) que comparten el mismo
        # número de cliente/cuenta, por lo que NO se puede agrupar por ese
        # número: se agrupa por CBU, que sí es único por producto.
        patron_cuenta = re.compile(
            r'^(.+?)\s+(\d{5,})\s+SUBCTA\s+(\d+)\s+SUC\s+(\d+)\s+CBU:\s*(.+)$'
        )
        secciones_raw = []
        for i, line in enumerate(lineas):
            m = patron_cuenta.match(line.strip())
            if m:
                tipo_cuenta = m.group(1).strip()
                cbu_limpio = m.group(5).replace(" ", "").strip()
                secciones_raw.append({
                    "tipo_cuenta": tipo_cuenta, "cuenta": m.group(2),
                    "subcta": m.group(3), "suc": m.group(4),
                    "cbu": cbu_limpio, "start_idx": i
                })
        ...
        # Agrupar secciones por CBU (identifica de forma única cada cuenta/producto)
        from collections import OrderedDict
        cuentas_agrupadas = OrderedDict()
        for sec in secciones_raw:
            clave = sec["cbu"]
            if clave not in cuentas_agrupadas:
                cuentas_agrupadas[clave] = []
            cuentas_agrupadas[clave].append(sec)

        # 3. REGEX MOVIMIENTOS (los saldos pueden terminar en "-" cuando son negativos)
        ...
        # 4. PROCESAR CADA CUENTA (agrupada por CBU)
        output = io.BytesIO()
        wb = Workbook()
        wb.remove(wb.active)
        total_movimientos = 0
        ...
        for cbu_clave, secciones in cuentas_agrupadas.items():
            tipo_cuenta = secciones[0]["tipo_cuenta"]
            numero_cuenta = secciones[0]["cuenta"]
            ...
            # Nombre de cuenta legible para la hoja (el número de cuenta solo no
            # alcanza porque distintos productos comparten el mismo número)
            tipo_upper = tipo_cuenta.upper()
            if "CORRIENTE" in tipo_upper:
                tipo_short = "CTA CTE"
            elif "AHORRO" in tipo_upper:
                tipo_short = "CAJA AHORRO"
            else:
                tipo_short = tipo_cuenta
            if "DOLAR" in tipo_upper or "USD" in tipo_upper:
                tipo_short += " USD"
            cuenta_id = f"{tipo_short} {numero_cuenta}"
